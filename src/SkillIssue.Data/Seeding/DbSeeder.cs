        ...
        [
            new Bug
            {
                ...
                [
                    ...
                    new HintTier { Order = 3, Label = "File & Line", Content = "In MoreLinq/CountMethods.cs, QuantityIterator ends by returning count > min && count <= max. The lower bound uses > instead of >=, so a count exactly equal to min (AtLeast(n) on n elements, or Exactly(n) on n) is rejected. Change it to return count >= min && count <= max;." }
                ]
            }
        ]
    };
